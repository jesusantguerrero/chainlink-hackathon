...
// Taken from https://github.com/smartcontractkit/hardhat-starter-kit
...
export const networkConfig: Record<string | number, INetworkConfig> = {
  default: {
    name: "hardhat",
...
export const autoFundCheck = async (
  ...
  const balance = await hre.ethers.BigNumber.from(balanceHex._hex).toString();
  const contractBalanceHex = await linkTokenContract.balanceOf(contractAddr);
  const contractBalance = await hre.ethers.BigNumber.from(
